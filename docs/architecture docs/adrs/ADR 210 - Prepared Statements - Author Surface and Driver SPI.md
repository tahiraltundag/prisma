...
A SQL-builder callback ends with `.build()`, returning a plan; raw SQL callbacks also return plans. `Row` is derived from that plan's row type. An ORM facade callback ends with `.prepared.all()` or `.prepared.first()`, returning a `Preparable<DbRow, Result>` containing a SQL plan and a required result consumer rather than executing an ordinary terminal. ORM predicates preserve non-nullable scalar placeholders in shorthand, callback, relation/include and terminal filters, including fixed lists of individual operands. ORM equality and inequality use declaration nullability retained on the frozen `PreparedParamRef` AST node to select null-safe comparisons before SQL lowering. PostgreSQL renders `IS NOT DISTINCT FROM` / `IS DISTINCT FROM`; SQLite renders `IS` / `IS NOT`. Other structured comparisons reject nullable prepared operands. Shared ORM predicate binding applies these semantics to nested expressions and structured interoperability inputs, without changing SQL-builder equality. Raw SQL remains opaque, including interpolation payloads: it is neither rewritten nor rejected based on nullable parameters. Literal-null ORM filters retain their null checks. Root and nested ORM pagination accepts SQL's existing non-nullable numeric expression operands, including distinct and scalar/combine include refinements. First-row terminals replace any earlier limit with `1`, retaining unused-declaration validation for overwritten placeholders. ORM aggregate/mutation preparation and dynamic parameter lists are not part of this row-terminal surface.
