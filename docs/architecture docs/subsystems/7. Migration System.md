...
A command that advances a ref (`db sign`, `db init`, `db update`, `migrate --advance-ref`) writes the store entry for the hash it advances to before writing the pointer. The entry's `contract.d.ts` is rendered from the `contract.json` being snapshotted, through the control client's `renderContractDts`, never copied from the emitted `contract.d.ts` next to the project's `contract.json`: the declarations a `migration.ts` type-checks against are then the declarations of that snapshot's JSON by construction. The render runs before any migration is applied or any marker is written, so a contract whose types cannot be rendered refuses the command (`CONTRACT.VALIDATION_FAILED` when the family rejects it, `CONTRACT.TYPES_RENDER_FAILED` when the emitter does) instead of leaving the database migrated with no ref and no snapshot.
...
- `prisma migration new [--from <hash> --to <hash>] --name <slug>` — scaffold an empty `migration.ts` for hand-authoring.
