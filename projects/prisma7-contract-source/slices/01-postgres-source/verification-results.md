...
3. **JSON literal defaults on JSON columns** (same files; test `test/interpreter.defaults.json-literal.test.ts`). For `pg/json@1`, `pg/jsonb@1`, `sqlite/json@1`, a string literal is JSON text; invalid text is `PSL_INVALID_JSON_DEFAULT`. Item 2 (JSON half) of `handoffs/remove-dbgenerated.md`, pulled forward.
