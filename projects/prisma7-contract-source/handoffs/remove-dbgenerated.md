...
## Decision required before starting (operator)

Research on 2026-09-14 (recorded in `spec.md` § Product findings) found two facts that bear on item 3 below and need the operator's call, not the implementer's:

- Prisma 8 already carries opaque target SQL in the contract in three places, all content-addressed under ADR 234 and ADR 244: `@@index(expression:, where:)`, `@@check(expression:)`, and Postgres RLS policy `using:`/`withCheck:`. ADR 244 states the rule: the target writes SQL, the family writes names, identity is the hash. Column defaults are the one raw-SQL site compared by normalised text instead of by wire name.
- TypeScript authoring already exposes a raw expression default, `.defaultSql(expression)` (`packages/2-sql/2-authoring/contract-ts/src/contract-dsl.ts:320-324`), producing the same `{ kind: 'function', expression }` arm `dbgenerated` produces. Removing the PSL spelling without deciding about `.defaultSql` leaves the two authoring surfaces disagreeing.

The operator chooses one of:

- **(A) No raw-expression default anywhere.** Item 3 stands as written, and `.defaultSql` is removed from TS authoring in the same slice, with the same treatment for the contracts that use it (grep `defaultSql` across `packages/`, `examples/`, `test/`).
- **(B) Implement ADR 129 for defaults.** The design already exists and was never built: ADR 129 "Template-Tagged Literals for Extensions" specifies a qualified tag followed by a backtick literal (`pg.sql\`gen_random_uuid()\``), no interpolation, core canonicalisation (line endings, dedent, blank-line trim, 64 KiB limit, `${` rejected), routing to the owning pack, and a contract encoding with a `bodyHash` that participates in diffing. It rejected fenced code blocks and triple quotes as harder to parse deterministically. Nothing implements it today: the tokenizer (`packages/1-framework/2-authoring/psl-parser/src/tokenizer.ts`) has no backtick token and no package defines a tagged-literal node. Under (B), a default expression is spelled `@default(pg.sql\`(now() + '00:03:00'::interval)\`)`, stored and compared by its `bodyHash` under ADR 234/244's wire-name rule, and `.defaultSql` becomes the TS form of the same node. This is a feature: tokenizer and parser support for the tagged literal, the AST node ADR 129 defines, the pack routing, the default lowering, and verify by hash. It should be specified as its own slice before this brief's items 3 and 4 run; items 1, 2, 5 to 7 still apply. Note for that slice: `@@index(expression:)`, `@@check(expression:)`, and the RLS predicates were built as plain string arguments rather than ADR 129 literals, so ADR 129's "single mechanism" claim is currently false and the slice should say whether those three migrate to tagged literals or ADR 129 is amended to admit string arguments.

Until the operator answers, do not start item 3 or 4.
...
3. **Arbitrary expressions, per the decision above.** Under (A): `(now() + '00:03:00'::interval)` has no Prisma 8 spelling. `contract infer` omits such a default and prints the family's existing `// Raw default: <expression>` comment (`packages/2-sql/9-family/src/core/psl-contract-infer/default-mapping.ts:15-31`, today unreachable on Postgres only because `postgres-default-mapping.ts` always answers with `dbgenerated`); `db verify` then reports the live default as a not-expected finding in strict mode and tolerates it in lenient mode, which is the honest state. The Supabase contract drops that one default with a note in the pack's README; if the pack owners want it, they ask for a feature.
