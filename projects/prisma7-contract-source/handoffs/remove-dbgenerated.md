...
- **(B) A designed opaque-expression default under ADR 244.** A first-class spelling for a database-expression default, named and content-addressed like `@@check`, replacing `dbgenerated`'s text-compare semantics with the wire-name rule; `.defaultSql` becomes the TS form of the same feature. This is a feature with its own ADR, not a rename; it should be specified before implementation and this brief's items 1, 2, 4 to 7 still apply on top of it.
